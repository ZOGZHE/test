...
// 命名空间：用于区分不同模块的代码，避免命名冲突
namespace WoolyPath
{
    // 加载菜单控制器：负责管理游戏加载界面的显示与逻辑
    public class LoadingMenu : MonoBehaviour
    {
        public static LoadingMenu instance { get; private set; }
        [Header("加载条组件")] // 在Inspector面板中为这组变量添加标题，方便区分
        ...
        [SerializeField] float loadDuration = 0.4f;         // 模拟加载的总时长
        ...
        {
            // 检查必要组件是否已在Inspector中赋值，未赋值则报错提示
           ...
           // _maxFillWidth = loadingFill.rectTransform.rect.width;
           _maxFillWidth = 941f;
        ...
        public IEnumerator SimulateLoadingProcess()
        {
            ...
            // 加载完成后等待0.5秒（让用户看清100%的状态），再切换到游戏运行状态
