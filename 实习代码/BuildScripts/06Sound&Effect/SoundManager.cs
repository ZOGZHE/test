...
namespace ConnectMaster
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        private Queue<AudioSource> _sfxPool = new Queue<AudioSource>();
        private List<AudioSource> _activeSfxSources = new List<AudioSource>(); // 标记正在使用的音效
        [Header("音效池配置")]
        public int poolInitialSize = 15; // 初始池大小（适度增加）
        public int poolMaxSize = 50;     // 池最大容量（根据项目音效数量调整）
        ...
        public AudioSource PlaySound(string soundKey, Vector3 position = default, Transform parent = null)
        {
            ...
            // 2. 从池获取AudioSource（池满则替换最久未使用的音效）
            AudioSource audioSource = null;
            if (_sfxPool.Count > 0)
            {
                audioSource = _sfxPool.Dequeue(); // 从池取出可用对象
            ...
            else
            {
                ...
                // 取出列表第一个（最久未使用）的音效强制回收
                AudioSource oldestSource = _activeSfxSources[0];
                _activeSfxSources.RemoveAt(0);
                // 停止并重置该音效
                oldestSource.Stop();
                oldestSource.clip = null;
                oldestSource.gameObject.SetActive(false);
                oldestSource.transform.parent = _sfxParent;
                audioSource = oldestSource;
                Debug.LogWarning($"音效池已满，替换最久未使用的音效以播放[{soundKey}]！");
            }
            ...
            if (!soundData.isLoop)
            {
                StartCoroutine(RecycleSfxCoroutine(audioSource, soundData));
            }

            // 调试日志：监控池状态
            //Debug.Log($"播放音效[{soundKey}]，池状态：可用={_sfxPool.Count} 活跃={_activeSfxSources.Count} 总={_sfxParent.childCount}");
            return audioSource;
        }
        #endregion
        ...
        #region 工具方法（音量配置与同步）
        /// <summary>音效播放完成后回收至池（优化：监听播放状态+超时保护）</summary>
        private IEnumerator RecycleSfxCoroutine(AudioSource audioSource, SoundData soundData)
        {
            // 安全校验：避免对象已被手动销毁
            if (audioSource == null || audioSource.gameObject == null) yield break;

            // 超时保护：最多等待 Clip长度 + 1秒，避免无限等待
            float maxWaitTime = soundData.audioClip.length + 1f;
            float elapsedTime = 0f;

            // 等待音效播放结束（每帧检查，避免Clip长度不准的问题）
            while (audioSource.isPlaying && elapsedTime < maxWaitTime)
            {
                yield return null;
                elapsedTime += Time.deltaTime;
            }

            // 额外容错：避免协程残留（比如音效被手动Stop）
            yield return new WaitForEndOfFrame();

            // 移除活跃标记
            if (_activeSfxSources.Contains(audioSource))
            {
        ...
        public void UpdateSfxVolume(float volume)
        {
            ...
            foreach (Transform child in _sfxParent)
            {
                ...
                // 从对象名称提取soundKey（格式：Sound_{soundKey}）
                string objName = child.name;
                if (objName.StartsWith("Sound_"))
                {
                    string soundKey = objName.Substring("Sound_".Length);
                    if (_soundDict.TryGetValue(soundKey, out SoundData data) && !data.isBgm)
                    {
                        source.volume = data.volume * sfxVolume;
                    }
                }
